...
#     this function also takes a list of audio files and concatenates them into a single video file.
#
...
def video_editor(video_files: list[str], audio_files: list[str], output_filename: str):
    ...
    for idx, video in enumerate(video_files):
        try:
            ...
            # Load the corresponding audio file (if available)
            if idx < len(audio_files):
                print("Adding audio file: ", audio_files[idx].title())
                audio = AudioFileClip(audio_files[idx])
                # Set the audio to the video clip
                clip_cropped = clip_cropped.set_audio(audio)
